...
class ioInterface:
    ...
    def init(self):
        ...
        status = 0
        for i in range(8):
            status |= c.dio_config(self.c_con, channel.PORT_1_SUBDEVICE, i + channel.PORT_1_CHANNEL_OFFSET,
                                   channel.PORT_1_DIRECTION)
        status |= c.dio_config(self.c_con, channel.PORT_2_SUBDEVICE, i + channel.PORT_2_CHANNEL_OFFSET,
                               channel.PORT_2_DIRECTION)
        status |= c.dio_config(self.c_con, channel.PORT_3_SUBDEVICE, i + channel.PORT_3_CHANNEL_OFFSET,
                               channel.PORT_3_DIRECTION)
        status |= c.dio_config(self.c_con, channel.PORT_4_SUBDEVICE, i + channel.PORT_4_CHANNEL_OFFSET,
