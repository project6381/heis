...
from config_parameters import MASTER_TO_MASTER_PORT, N_ELEVATORS, MY_ID
...
class MasterHandler:
    ...
    def process_slave_event(self, slave_message):
        ...
        if self.active_master() == MY_ID:
            self.__clear_completed_orders(slave_message['direction'], slave_message['last_floor'],
                                          slave_message['next_floor'])
            self.__update_elevator_position(slave_message['slave_id'], slave_message['last_floor'],
                ...
                if (self.__orders_up[floor] == 1) and ((self.__assigned_orders_up[floor] == 0) or (
                    ...
                    for elevator in range(0, N_ELEVATORS):
                        ...
                        if self.__slaves_online[elevator] == 0:
                            ...
                            elevator_priority_up[elevator] = 20 + N_FLOORS * 20 + (N_FLOORS - abs(
                                self.__elevator_positions[elevator][LAST_FLOOR] - floor)) * 10 + randint(0, 9)

                        elif self.__elevator_positions[elevator][DIRECTION] == DIRN_STOP:

                            elevator_priority_up[elevator] = 10 + N_FLOORS * 10 + (N_FLOORS - abs(
                                self.__elevator_positions[elevator][LAST_FLOOR] - floor)) * 10 + randint(0, 9)
                        else:
                    ...
                    for elevator in range(0, N_ELEVATORS):
                        ...
                        elif (elevator_priority_up[elevator] > elevator_priority_up[
                                self.__assigned_orders_up[floor] - 1]) and (self.__slaves_online[elevator] == 1):
                ...
                if (self.__orders_down[floor] == 1) and ((self.__assigned_orders_down[floor] == 0) or (
                    ...
                    for elevator in range(0, N_ELEVATORS):
                        ...
                        if self.__slaves_online[elevator] == 0:
                            ...
                            elevator_priority_down[elevator] = 20 + N_FLOORS * 20 + (N_FLOORS - abs(
                                self.__elevator_positions[elevator][LAST_FLOOR] - floor)) * 10 + randint(0, 9)

                        elif (self.__elevator_positions[elevator][DIRECTION] == DIRN_STOP):

                            elevator_priority_down[elevator] = 10 + N_FLOORS * 10 + (N_FLOORS - abs(
                                self.__elevator_positions[elevator][LAST_FLOOR] - floor)) * 10 + randint(0, 9)
                        else:
                    ...
                    for elevator in range(0, N_ELEVATORS):
                        ...
                        elif (elevator_priority_down[elevator] > elevator_priority_down[
                                self.__assigned_orders_down[floor] - 1]) and (self.__slaves_online[elevator] == 1):
    ...
    def __alive_thread(self):
        try:
            ...
            try:
                ...
                print "MasterHandler.__alive_message_handler: Failed setting up UDP sockets!"
                print error
                interrupt_main()
            ...
            while True:
                try:
                    ...
                    print "MasterHandler.__alive_message_handler: udp.recvfrom(1024) failed!"
                    print error
