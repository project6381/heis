					...
					for floor in range(0,N_FLOORS):
						for button in range(0,N_BUTTONS):
								if self.__elevator_orders[floor][button] == 1:
									...
									if (last_floor == next_floor) and (direction != DIRN_DOWN) and (next_floor <= highest_elevator_order):
										next_floor = floor
										next_button = button
									elif (last_floor == next_floor) and (direction != DIRN_UP) and (next_floor >= lowest_elevator_order):
										next_floor = floor
										next_button = button
